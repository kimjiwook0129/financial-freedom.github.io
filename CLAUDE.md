...
src/
  lib/
    ...
    hash.ts              # URL hash encode/decode (Base64url, pipe-delimited state)
...
- Hash load/share event handlers
...
| `hash.ts` | `encodeState`/`decodeState` for URL hash persistence. Format: `#1\|lang\|currency\|field1\|...\|fieldN` (Base64url). |
